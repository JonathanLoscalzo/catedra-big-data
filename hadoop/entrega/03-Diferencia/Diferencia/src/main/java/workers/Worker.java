...
public class Worker extends Configured implements Tool {
	...
	private Job setupJob(Configuration conf, String[] args) throws IOException{		
	    ...
	    job.setCombinerClass(DiffCombiner.class);
	   
	    // job.setInputFormatClass(TextInputFormat.class);
	    // job.setOutputFormatClass(TextOutputFormat.class);
	    
	    MultipleInputs.addInputPath(job, new Path("Conjuntos/"+args[0]), TextInputFormat.class, DiffMapperFromA.class);
	    MultipleInputs.addInputPath(job, new Path("Conjuntos/"+args[1]), TextInputFormat.class, DiffMapperFromB.class);
